...
导出列不能只封装成“字典 key + 表头字符串”。参考 Ares `BackgroundReportService` 中 `ExportColumn.Field`、`ExportColumn.Title` 和 `ExportColumnValue` 的分层思路，Atlas 第一版采用更明确的列和值模型，但仍保留后台任务、权限、文件存储等 Atlas 自身边界。
...
1. `Field` 是稳定机器字段名，用于匹配导出行数据，不能使用中文列名承担字段标识。
2. `Title` 是展示列名，用于 CSV header 或未来 Excel header，可以在 provider 内固定，也可以由服务端配置或本地化解析。
3. `ValueKind` 和 `Format` 由列定义或单元格值提供，writer 根据它们进行数字、日期和时间格式化。
4. `Hidden` 列和空 `Title` 列不写入文件，便于支持服务端列白名单和占位列。
5. `IsSensitive` 只表达列级风险元数据，是否明文导出仍必须由权限和审计策略决定。
6. `Order` 用于稳定列顺序；没有显式排序时保留 provider 声明顺序。
7. 可见列的 `Field` 必须唯一，重复字段应在 writer 阶段失败，而不是静默输出错误文件。
8. 前端自定义列只允许传 `Field` 列表，服务端必须用 provider 的列目录做白名单校验，不能信任前端传入的 `Title`。
9. request 模型实现 `IExportColumnSelection` 后，`ExportTaskProvider<TQuery>` 会自动按 `SelectedFields` 解析最终列。
10. `SelectedFields` 为空时导出 provider 默认列；非空时保留前端字段顺序，但仍使用服务端列元数据。
11. 未知字段、隐藏字段、重复字段必须失败，不能静默忽略。
...
导出 request 不应把所有搜索字段直接平铺在根对象上。根对象属于导出框架选项，主要承载 `Format`、`SelectedFields` 等导出行为；业务筛选条件放入强类型 `Criteria`。

这样有三个好处：

1. 搜索条件不被简化成单个 `Keyword`，业务模块可以定义任意受控条件，例如状态、时间范围、门店、价格区间、排序方式等。
2. 框架可以统一识别导出格式和列选择，controller 不必手动拆分导出选项。
3. provider 仍然把 `Criteria` 映射到 Repository 或 QueryService，不引入任意 SQL 或通用表达式 DSL。

导出分页和前端列表分页必须分开：

1. `ReadPageAsync` 的 `pageSize` 是导出执行批大小，由 `Exporting:DefaultPageSize` 和 `Exporting:MaxPageSize` 控制，不由前端列表页大小决定。
2. 前端提交导出时只传筛选条件、排序、格式和列选择，不传普通列表的 `PageIndex`、`PageSize`。
3. provider 调用 QueryService 时应覆盖分页参数，使用 `ReadPageAsync` 收到的 `pageIndex`、`pageSize`。
4. 如果业务确实需要“只导出当前页”，应建模为单独导出场景或显式 `Limit` 语义，不能复用列表分页参数误伤全量导出。
...
6. 列选择、格式和业务 `Criteria` 一起进入查询快照，保证导出重试时复现同一次用户选择。
...
3. 查询条件和执行结果可以被审计。
4. 不同导出任务类型可以拥有完全不同的查询条件模型，且这些条件由业务模块映射到受控 QueryService。
...
1. 调用 `provider.ResolveColumnsAsync` 获取本次任务的最终列定义。
2. 只写出 `Hidden == false` 且 `Title` 非空的列。
3. 按 `Order` 和 provider 声明顺序稳定输出列。
4. 校验可见列 `Field` 唯一，避免字段重复导致数据错位。
5. 使用列定义和 `ExportCellValue` 中的 `ValueKind`、`Format` 执行格式化。
6. 不从数据行字典推断列顺序，不信任前端传入的列名。
...
2. 正确处理逗号、双引号、换行。
3. 列顺序来自 `ResolveColumnsAsync` 的最终列定义，并按 `Order` 稳定排序。
4. 日期、数字使用 payload 中的 `Culture`、`TimeZone` 以及列级 `Format`。
5. 可见列字段重复时失败，不能生成存在歧义的文件。
6. 数据行字典只负责按 `Field` 提供值，不负责决定 header 或列顺序。
7. writer 不应仅因为某页返回行数小于请求 `pageSize` 就结束；QueryService 可能有自己的最大页大小。应优先依据空页或 `TotalRows` 判断结束。
...
16. writer 调用 provider 解析最终列定义。
17. writer 分页调用 provider 读取数据。
18. writer 写入临时文件。
19. 写入成功后 commit 文件。
20. 更新 `ExportJobs` 为 `Ready`。
21. `BackgroundJobs` 标记 `Succeeded`。
22. 用户通过下载接口获取文件。
...
1. provider 列定义通过 `IsSensitive` 标记敏感列。
2. 默认导出脱敏值。
3. 只有具备高风险权限时可导出明文。
4. 明文导出必须写审计。
5. 如果支持用户选择列，服务端必须过滤掉当前权限不可导出的敏感列。
...
2. 新增核心抽象和 DTO。
3. 新增导出列和值模型，包括 `ExportColumn`、`ExportCellValue` 和 `ExportValueKind`。
4. 新增 `ExportJob` Global 实体和 EF configuration。
5. 新增 migration。
6. 实现 `ExportJobService`。
7. 实现 `ExportJobHandler`。
8. 实现 CSV writer。
9. 实现本地 file store。
10. 接入 DI 扩展。
...
13. CSV writer 按列定义输出 header、顺序、隐藏列和类型格式，重复可见 `Field` 会失败。

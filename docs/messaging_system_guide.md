...
```mermaid
flowchart LR
    A["业务服务<br/>OrderCommandService"] --> B["租户库事务"]
    B --> C["Orders"]
    B --> D["TenantOutboxMessages"]
    D --> E["TenantOutboxDispatcher"]
    E --> F["RabbitMQ"]
    F --> G["MassTransit Consumer"]
...
消费者通过 MassTransit `IConsumer<T>` 实现：
...
| 索引 | 说明 |
| --- | --- |
| `IX_TenantOutboxMessages_EventId` | 防止同一事件重复入 outbox。 |
...
消费者必须按 `MessageId + ConsumerName` 做幂等。可以参考：

```text
samples/Atlas.Sample.WebApi/Consumers/OrderPlacedEventConsumer.cs
```

消费者模板：

```csharp
public sealed class InventoryChangedEventConsumer : IConsumer<InventoryChangedEvent>
{
    private const string ConsumerName = nameof(InventoryChangedEventConsumer);
    private readonly ITenantDbContextFactory _dbContextFactory;

    public async Task Consume(ConsumeContext<InventoryChangedEvent> context)
    {
        var message = context.Message;
        ...
        var consumed = await db.Set<TenantInboxMessage>()
            .AnyAsync(
                x => x.MessageId == messageId && x.ConsumerName == ConsumerName,
                context.CancellationToken);
        ...
        // Execute business side effects here.

...
namespace Atlas.Services.Tenant.Runtime.Provisioning;
...
public sealed class TenantProvisioningService : ITenantProvisioningService
{
        ...
        CancellationToken ct)
    {
        ...
        try
        {
            ...
            await tenantDb.Set<AuditEvent>().AddAsync(new AuditEvent
            {
                TenantId = tenant.Id,
                StoreId = store.Id,
                Category = "TenantManagement",
                Action = "TenantProvisioning",
                Outcome = AuditEventOutcome.Succeeded,
                EntityType = nameof(GlobalTenant),
                EntityId = tenant.Id,
                Metadata = $"Domain={tenant.Domain};HeadquartersStoreCode={store.Code}"
            }, ct);
            await tenantDb.SaveChangesAsync(ct);
